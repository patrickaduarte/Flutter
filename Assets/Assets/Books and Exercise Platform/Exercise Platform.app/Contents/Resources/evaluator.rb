...
class Evaluator
    ...
    def run(code)
        ...
        timeout_function = "function timeout() { perl -e 'alarm shift; exec @ARGV' \"$@\"; }"

        cmd = "#{timeout_function}; timeout #{timeLimit + 1}s #{swift} '#{tempFilePath}' #{sdkPathOption} 2> .tempFile2" 
        # puts "#{cmd}"
        ...
        if info != nil
            output_values = info.split("!!!!!!!!!!#")

            if output_values.count > 0 
                output_values = output_values[1..-1].map { |value|
                    value[0..-2]
                }
            end

            info = output_values.map { |line|
    ...
    def evaluateTestFile
        ...
        evaluation_results = testFile.process(results)

        ########## Save Data in store for platform
        status = evaluation_results[:status]

        save_code_for self.testFile._uid, evaluation_results[:code]

        if status == 1 
            solved_exercise self.testFile._uid
        end
        ##########
