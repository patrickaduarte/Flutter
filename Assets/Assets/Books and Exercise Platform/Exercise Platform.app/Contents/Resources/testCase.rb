...
class TestCase
    ...
    def populate_binding _code, cool_binding
        ...
        import_lines = extractImports! code
        ...
        expectedFunctions.each do |expectedFunc, options|
            ...
            to_eval += "has[#{expectedFunc.inspect}] = #{has_func}\n"
            to_eval += "has[#{fname.inspect}] = #{has_func}\n"
        end
    ...
    def extractImports! _code 
        imports = _code.lines.select { |line|
            line["import "]
        }

        imports.each do |import_lib|
            _code.gsub!(import_lib, "")
        end
