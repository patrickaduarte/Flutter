...
def get_ast(code)
    ...
    Open3.popen3("#{swiftc} -dump-ast #{path}") do |stdin, stdout, stderr|
        raw_ast = stderr.read
    end

    # import Foundation fix
	raw_ast = raw_ast.lines.select { |line|
		line["<unknown>:0:"] == nil
	}.join    
...
def parse_ast(raw_ast)
    ...
    # fix to ignore warnings and other messages
    ...
    raw_ast.force_encoding("utf-8").lines.select { |line|
        line.strip.length != 0
    }.each do |line|
        if line.strip == ")"
            stack.pop
